...
#[derive(Optic, Default, Clone, Copy, Debug, PartialEq, Eq, Serialize, Deserialize)]
pub struct FocusedJoinState {
    #[jungle(focus)]
    focused: i32,
}

pub struct FocusedJoinLeftSpec;
#[jungle::act]
impl Act for FocusedJoinLeftSpec {
    type Effect = TimedValueEffect;
    type Input = ();
    type Output = ();

    fn emit(_state: &i32, _input: Self::Input) -> (u64, i32) {
        (0, 10)
    }

    fn absorb(_state: &mut i32, output: EffectCompletion<Self::Effect>) -> Self::Output {
        let _ = output.expect("focused join left should succeed");
    }
}

pub struct FocusedJoinRightSpec;
#[jungle::act]
impl Act for FocusedJoinRightSpec {
    type Effect = TimedValueEffect;
    type Input = ();
    type Output = ();
    ...
    fn absorb(_state: &mut i32, output: EffectCompletion<Self::Effect>) -> Self::Output {
        let _ = output.expect("focused join right should succeed");
    }
}

pub struct FocusedJoinMergeSpec;
#[jungle::act]
impl Act for FocusedJoinMergeSpec {
    type Effect = Noop;
    type Input = ((), ());
    type Output = ();
    ...
    fn absorb(_state: &mut i32, output: EffectCompletion<Self::Effect>) -> Self::Output {
        output.expect("focused join merge should succeed");
    }
}

pub struct FocusedJoinTailSpec;
#[jungle::act]
impl Act for FocusedJoinTailSpec {
    type Effect = TimedValueEffect;
    type Input = ();
    type Output = ();
    ...
    fn absorb(state: &mut i32, output: EffectCompletion<Self::Effect>) -> Self::Output {
        let value = output.expect("focused join tail should succeed");
        *state = state.saturating_add(value);
    }
}

pub struct FocusedJoinPostMergeRestSpec;
#[jungle::act]
impl Act for FocusedJoinPostMergeRestSpec {
    type Effect = Sleep;
    type Input = ();
    type Output = ();

    fn emit(_state: &i32, _input: Self::Input) -> Duration {
        Duration::from_millis(1)
    }

    fn absorb(_state: &mut i32, output: EffectCompletion<Self::Effect>) -> Self::Output {
        output.expect("focused post-merge rest should succeed");
    }
}

#[derive(Flow)]
#[jungle(focus = i32)]
pub struct FocusedJoinNoopTailFlow(
    Join<Step<FocusedJoinLeftSpec>, Step<FocusedJoinRightSpec>>,
    Step<FocusedJoinMergeSpec>,
    Step<FocusedJoinPostMergeRestSpec>,
    Step<FocusedJoinTailSpec>,
);

pub struct FocusedJoinNoopTailAnimal;

#[jungle::animal(id = 9, generation = 0)]
impl Animal for FocusedJoinNoopTailAnimal {
    type State = FocusedJoinState;
    type Seed = FocusedJoinState;
    type Journey = FocusedJoinNoopTailFlow;
}
...
#[tokio::test]
async fn focused_join_then_noop_then_tail_does_not_hang() {
    let mut executor =
        ContextExecutor::<_, FocusedJoinNoopTailAnimal>::new(std::sync::Arc::new(()), FocusedJoinState::default());

    let run = tokio::time::timeout(Duration::from_secs(2), async {
        while !executor.is_complete() {
            let request = executor
                .next_executable_request(())
                .expect("focused join flow should yield a request");
            let completion = request
                .run()
                .await
                .expect("focused join effect should run");
            executor
                .complete_serialized(completion)
                .expect("focused join completion should advance flow");
        }
    })
    .await;
    run.expect("focused join flow should not hang");
    assert_eq!(executor.state().focused, 1);
}
