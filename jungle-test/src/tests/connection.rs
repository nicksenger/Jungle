...
#[tokio::test]
async fn poll_owner_wake_allows_current_lease_owner_to_claim_stale_wake() {
    let tempdir = tempfile::tempdir().expect("temp dir should be created");
    let db_path = tempdir.path().join("jungle.fjall");
    ...
    client
        .heartbeat_journey_lease(journey_id, old_owner, 10_000)
        .await
        .expect("heartbeat_journey_lease should succeed for initial owner");
    ...
    let before_transfer = client
        .poll_owner_wake(new_owner)
        .await
        .expect("poll_owner_wake should succeed");
    assert!(
        before_transfer.is_none(),
        "new owner should not receive wake before lease transfer"
    );

    client
        .heartbeat_journey_lease(journey_id, new_owner, 10_000)
        .await
        .expect("heartbeat_journey_lease should succeed for transferred owner");

    let claimed = client
        .poll_owner_wake(new_owner)
        .await
        .expect("poll_owner_wake should succeed after lease transfer")
        .expect("new owner should be able to claim wake after lease transfer");
    assert_eq!(claimed.journey_id, journey_id);
    assert_eq!(claimed.timer_id, timer_id);
