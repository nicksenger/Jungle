...
impl<State, Carrier, F> ArgputForState<State> for jungle_types::FocusedBoundFlow<Carrier, F>
where
    F: ArgputForState<State>,
{
    type Carry = <F as ArgputForState<State>>::Carry;
...
where
    (): __inception_running::FieldsInput<F>,
    <() as __inception_running::FieldsInput<F>>::In: SplitStateCarry<State>,
{
    type Carry =
        <<() as __inception_running::FieldsInput<F>>::In as SplitStateCarry<State>>::Carry;
}
...
impl<State, Carrier, F> JoinFocusMarker<State> for jungle_types::FocusedBoundFlow<Carrier, F>
where
    State: Clone,
    Carrier: jungle_types::Aspect<State>,
    <Carrier as jungle_types::StateCarrier<State>>::Focus: Clone,
{
    const ENABLED: bool = true;

    fn merge_into(target: &mut State, mut branch_state: State) {
        let focused = <Carrier as jungle_types::StateCarrier<State>>::focus(&mut branch_state).clone();
        *<Carrier as jungle_types::StateCarrier<State>>::focus(target) = focused;
    }
}
...
impl<State, Carrier, F> FlowCarry<State> for jungle_types::FocusedBoundFlow<Carrier, F>
where
    F: FlowCarry<State>,
{
    type In = <F as FlowCarry<State>>::In;
...
#[inception::primitive(property = jungle_types::JungleDynFlow)]
...
where
    ...
    P: jungle_types::Predicate<(State, <L as ArgputForState<State>>::Carry)> + 'static,
{
    type Output = DynFlow<State>;
    ...
    fn push_steps(mut steps: DynFlow<State>) -> Self::Output {
        ...
        let choose_left = Box::new(
            |state: &State, input: &<L as ArgputForState<State>>::Carry| {
                <P as jungle_types::Predicate<(State, <L as ArgputForState<State>>::Carry)>>::eval(&(
                    state.clone(),
                    input.clone(),
                ))
            },
...
where
    State: Send + 'static,
    C: for<'a> jungle_types::Predicate<(&'a State, &'a In)> + 'static,
    ...
    F: BuildFlow<DynFlow<State>, Output = DynFlow<State>> + FlowCarry<State, In = In> + 'static,
{
    ...
    fn push_steps(mut steps: DynFlow<State>) -> Self::Output {
        ...
        let should_continue = Box::new(|state: &State, input: &In| {
            <C as jungle_types::Predicate<(&State, &In)>>::eval(&(state, input))
        });
...
#[inception::primitive(property = jungle_types::JungleDynFlow)]
impl<State, Carrier, F> BuildFlow<DynFlow<State>> for jungle_types::FocusedBoundFlow<Carrier, F>
where
    F: BuildFlow<DynFlow<State>, Output = DynFlow<State>>,
{
    type Output = DynFlow<State>;
...
impl<Context, State, Carrier, F> BuildFlowWithContext<(Arc<Context>, DynFlow<State>)>
    for jungle_types::FocusedBoundFlow<Carrier, F>
where
    F: BuildFlowWithContext<
        (Arc<Context>, DynFlow<State>),
        Output = (Arc<Context>, DynFlow<State>),
