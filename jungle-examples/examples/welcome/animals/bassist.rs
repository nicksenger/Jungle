...
mod tests {
    ...
    struct LocalBassJoinTailAnimal;
    #[jungle::animal(id = 0, generation = 0)]
    impl Animal for LocalBassJoinTailAnimal {
    ...
    #[derive(Animals)]
    struct LocalBassJoinTailAnimals(LocalBassJoinTailAnimal);

    struct LocalBassJoinTailZoo;
    impl Ecosystem for LocalBassJoinTailZoo {
        const NAME: &'static str = "welcome-bass-join-tail-zoo";
        type Animals = LocalBassJoinTailAnimals;
    }
    ...
    async fn join_then_tail_streams_events_and_completes_with_local_client() {
        let namespace = format!("welcome-bass-join-tail-repro-{}", uuid::Uuid::new_v4());
        let client = LocalClient::builder()
            .namespace(&namespace)
            .build()
            .await
        ...
        let journey_id = client
            .start_journey::<LocalBassJoinTailAnimal>(
        ...
        let worker = JungleWorker::new(LocalBassJoinTailZoo, client.clone());
