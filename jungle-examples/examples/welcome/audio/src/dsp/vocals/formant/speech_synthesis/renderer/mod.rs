...
fn create_frames(
    pitch: u8,
    phonemes: &[super::parser::Phoneme],
    frequency_data: &FrequencyData,
) -> Vec<Frame> {
    let mut frames = Vec::new();

    for phoneme in phonemes {
        if phoneme.index == super::parser::PHONEME_PERIOD {
            add_inflection(Inflection::Falling, &mut frames);
        } else if phoneme.index == super::parser::PHONEME_QUESTION_MARK {
