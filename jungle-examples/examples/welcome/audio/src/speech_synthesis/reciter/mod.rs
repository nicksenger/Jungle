...
mod tests {
    ...
    fn from_file() {
        ...
        let testcases: Vec<TestCase> =
            serde_json::from_str(contents).expect("Could not deserialize reciter test file");

        let max_line_length: usize = 30;

        for (index, testcase) in testcases.iter().enumerate() {
            print!(
                "\x1b[90m{:3} \x1b[94m{:max_line_length$}\x1b[90m -> \x1b[0m",
                index, testcase.text
            );
            ...
            match &result {
                ...
                Err(err) => {
                    println!(
                        "\x1b[1;91mFAIL\x1b[0;31m ({:?}) in \x1b[91m{:?}\x1b[0m",
                        err, duration
                    );
                }
            }
