...
The video is of the [welcome](./jungle-examples/examples/welcome/) example, which showcases some of the things possible in the `jungle` by representing Guns'n'Roses' 1982 hit single "Welcome to the Jungle" as a `jungle` `Flow` performed by `jungle` `Animal`s, and visualized using `jungle-vision`.
...
Instead of Workflow-as-Code (WaaC), you can think of the `jungle` as Workflow-as-Type (WaaaT).
