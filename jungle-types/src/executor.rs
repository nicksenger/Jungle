    ...
    F: FnOnce(&In) -> bool,
{
    if let Ok(either) = postcard::from_bytes::<Either<Serialized, Serialized>>(input) {
        ...
        if let Ok(carry) = postcard::from_bytes::<In>(&carry_bytes) {
            return Ok((fallback(&carry), carry));
        }
        // Fall through to try direct deserialization when the Either envelope
        // was a false positive (data that happens to parse as Either but is not
